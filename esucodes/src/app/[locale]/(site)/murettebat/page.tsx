...
type TeamMember = {
  ...
  skills: string[]; github_url: string | null; linkedin_url: string | null;
  order_index: number; username: string | null;
};
...
function MemberCard({ member, onOpen }: { member: TeamMember; onOpen: (m: TeamMember) => void }) {
  ...
  const cardStyle: React.CSSProperties = {
    ...
    boxShadow: hover ? `0 8px 40px ${color}18` : "0 2px 12px rgba(0,0,0,0.3)",
    ...
    <>
      ...
      <div style={{ padding: "32px 28px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", flexGrow: 1 }}>
        ...
        <div style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 16, borderTop: "1px solid var(--border-subtle)" }}>
          ...
          <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color }}>{t("profile")} <ArrowRight size={16} color={color} /></span>
        </div>
      </div>
    </>
  );
  ...
  if (member.username) {
    return (
      <Link
        href={`/murettebat/${member.username}`}
        onMouseEnter={() => setHover(true)}
        onMouseLeave={() => setHover(false)}
        style={{ ...cardStyle, textDecoration: "none" } as React.CSSProperties}
      >
        {inner}
      </Link>
    );
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 620, background: "var(--bg-secondary)", border: `1px solid ${color}30`, borderRadius: 24, padding: "36px", boxShadow: `0 0 80px ${color}20`, maxHeight: "88vh", overflowY: "auto", position: "relative" }}>
        ...
        {member.skills.length > 0 && (
          <div style={{ padding: 20, borderRadius: 14, background: "var(--glass-fill)", border: "1px solid var(--border-subtle)", marginBottom: 16 }}>
            <h3 style={{ fontSize: 14, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 14px" }}>{t("skills")}</h3>
        ...
        {(member.github_url || member.linkedin_url) && (
          <div style={{ display: "flex", gap: 10 }}>
            {member.github_url && <a href={member.github_url} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px", borderRadius: 10, background: "var(--glass-fill)", border: "1px solid var(--border-subtle)", color: "var(--text-secondary)", textDecoration: "none", fontSize: 13, fontWeight: 600 }}><Code2 size={15} /> {t("github")}</a>}
            {member.linkedin_url && <a href={member.linkedin_url} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px", borderRadius: 10, background: "var(--glass-fill)", border: "1px solid var(--border-subtle)", color: "var(--text-secondary)", textDecoration: "none", fontSize: 13, fontWeight: 600 }}><Link2 size={15} /> {t("linkedin")}</a>}
          </div>
        )}
      </div>
    </div>
...
export default function TeamPage() {
  ...
  const stats = [
    { value: members.length,     label: t("foundingMember"),  icon: Users },
    { value: projectCount + "+", label: t("activeProjects"),  icon: FolderOpen },
    ...
    <div>
      ...
      <section style={{ maxWidth: "var(--container-max)", margin: "0 auto", padding: "72px 24px" }}>
        ...
        <div style={{ padding: "48px", borderRadius: 24, textAlign: "center", background: "linear-gradient(135deg, rgba(129,140,248,0.10) 0%, rgba(34,211,238,0.07) 100%)", border: "1px solid rgba(129,140,248,0.25)" }}>
          <div style={{ fontSize: 36, marginBottom: 12 }}>👨‍🚀</div>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: "var(--text-primary)", margin: "0 0 10px" }}>{t("joinCta")}</h2>
          <p style={{ fontSize: 16, color: "var(--text-secondary)", margin: "0 0 24px", lineHeight: 1.65 }}>{t("joinCtaText")}</p>
